...
def read_links_lt(dfloc,o_list, concess_not_working):
    try:
    ...
    try:
        ...
        return(None)
def tariffs_runner_lt(df, thread_num=10):
    threads= []
    o_list= []
    o_list= []
    concess_not_working = []
    with ThreadPoolExecutor(max_workers=thread_num) as executor:
        for i in range(df.shape[0]):
            dfloc = df.loc[i]
            threads.append(executor.submit(read_links_lt, dfloc, o_list, concess_not_working))
            
    return [item for sublist in o_list for item in sublist], concess_not_working
def tariffs_all_lt(df, thread_num=10):
    t0, concess = tariffs_runner_lt(df, thread_num)
    t0 = pd.DataFrame(t0)
    return t0, concess
